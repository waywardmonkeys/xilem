...
impl Widget for Badged {
    ...
    fn layout(&mut self, ctx: &mut LayoutCtx<'_>, _props: &PropertiesRef<'_>, size: Size) {
        ...
        let Some(badge) = &mut self.badge else {
            let child_baseline = ctx.child_baseline_offset(&self.content);
            let child_bottom = content_size.height;
        ...
        let content_anchor = match self.placement {
            BadgePlacement::TopLeft => Point::new(0.0, 0.0),
            BadgePlacement::TopRight => Point::new(content_size.width, 0.0),
            BadgePlacement::BottomLeft => Point::new(0.0, content_size.height),
            BadgePlacement::BottomRight => Point::new(content_size.width, content_size.height),
        };

        let badge_origin = content_anchor + self.offset
            - Vec2::new(badge_size.width / 2.0, badge_size.height / 2.0);
